...
Create a new [API Route](https://nextjs.org/docs/app/building-your-application/routing/router-handlers) at `app/api/queues/email.js` and paste the following:

```typescript title="app/api/queues/email/route.ts"
import { Queue } from "quirrel/next13"

export const { POST, queue: emailQueue } Queue(
